		...
		<div class="container">
			...
			<div id="toggle" class="navbar-collapse collapse">
				...
				<ul class="navbar-nav ml-auto">
					<li class="nav-item dropdown mr-3"><c:if test="${!empty sessionScope.id}">
						...
						<div class="dropdown-menu">
							<c:choose>
								<c:when test="${!empty sessionScope.id}">
									<a class="dropdown-item" href="./MemberSelectDetail.me?id=${sessionScope.id}"> <i class="fas fa-user-shield"></i> 회원 정보
									</a>
								</c:when>
							</c:choose>
						</div> <c:choose>
							<c:when test="${empty sessionScope.id}">
								<li class="nav-item"><a href="./LoginView.me" class="nav-link"> <i class="fas fa-user-circle mr-sm-1"></i> 로그인
								</a>
							</c:when>
							<c:otherwise>
								<li class="nav-item"><a href="./LoginView.me" class="nav-link"> <i class="fas fa-user-times mr-sm-1"></i> 로그아웃
								</a>
							</c:otherwise>
						</c:choose>
				</ul>
				<ul class="navbar-nav py-3 mb-3 pl-3 pr-3">
					<li><a href="./index.jsp" class="navbar-brand text-white"> <i class="fa fa-home"></i>홈
					</a></li>
				</ul>
			</div>
		</div>
				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header">
							<h3 class="text-muted">
								<i class="fas fa-file-alt mr-sm-1"></i>검색글 목록
							</h3>
							<p align="right">
								<%--전체 글의 개수를 호출한다.--%>
							...
							<ul class="pagination justify-content-center">
								<c:choose>
									...
									<c:otherwise>
										<li class="page-item"><a
											href="./BoardSearchSelect.do?page=${page-1}&keyword=${sessionScope.keyword}&keyfield=${sessionScope.keyfield}" class="page-link"
										> 이전 </a></li>
									</c:otherwise>
								</c:choose>
								...
								<c:forEach var="start" begin="${startpage}" end="${endpage}" step="1">
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a
												href="./BoardSearchSelect.do?page=${start}&keyword=${sessionScope.keyword}&keyfield=${sessionScope.keyfield}" class="page-link"
											>${start} </a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
						...
						<nav class="navbar justify-content-end">
							...
							<div class="btn-group">
								<button type="button" class="btn btn-outline-info mr-sm-1" onclick="location.href='./BoardInsertView.do'">
									<i class="fas fa-edit mr-sm-1 mr-sm-1"></i> 글 등록
								</button>
							</div>
						</nav>
					</div>
